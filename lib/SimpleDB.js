...
function RecordType (simpleDB)
{
	...
	this._postAddRecordFunction = null;
	this._allRecords = null;
	this._recordRelations = [];
	this._recordReferenceProperties = [];
	...
	this._propertiesToStore = null;
	this._propertiesToNotStore = null;
}
...
SimpleDB.prototype.assignNextAvailableRecordIndex = function ()
{
	var result = this._nextAvailableRecordIndex;

	this._nextAvailableRecordIndex ++;
...
SimpleDB.prototype.newOrphanRecord = function (recordTypeIdentifier,pOptions)
{
    ...
    if (options.resolveRecordTypeIdentifier)
    	recordType = this.resolveRecordType (recordTypeIdentifier);
    else
	...
	if (isDefined (pOptions) && isDefined (pOptions.properties))
		copyProperties (pOptions.properties,record);
	...
	if (isDefined (recordType._postAddRecordFunction))
	{
		var postAddRecordFunction = recordType._postAddRecordFunction.bind (recordType._simpleDB);
		postAddRecordFunction (record);
	}
...
SimpleDB.prototype.addOrphanRecord = function (orphanRecord,pOptions)
{
	if (!orphanRecord._simpleDB.index > 0)
		orphanRecord._simpleDB.index = this.assignNextAvailableRecordIndex ();
...
SimpleDB.prototype.initialiseSimpleDB = function (storageKey,initialiseStructureFunction,pOptions)
{
	SimpleDB.apply (this);
	...
	initialiseStructureFunction (this);

	if (options.loadData)
		this.loadData ();
}
RecordType.prototype.addRecordReferenceProperty = function (propertyName,recordTypeIdentifier,isArray)
{
	var recordType = this;
	var simpleDB = recordType._simpleDB;

	var p = new RecordReferenceProperty ();
	p.propertyName = propertyName;
	p.recordType = simpleDB.resolveRecordType (recordTypeIdentifier);
	p.isArray = isArray;

	recordType._recordReferenceProperties.push (p);

	if (p.isArray)
		recordType [propertyName] = [];
	else
		recordType [propertyName] = null;
...
SimpleDB.prototype.saveManager = function ()
{
	...
	copyProperties (simpleDB,saveObject,['_nextAvailableRecordTypeIndex','_nextAvailableRecordIndex']);
...
SimpleDB.prototype.loadManager = function ()
{
	...
	copyProperties (saveObject,simpleDB,['_nextAvailableRecordTypeIndex','_nextAvailableRecordIndex']);
}
...
SimpleDB.prototype.loadRecords = function (recordType)
{
	var simpleDB = this;
	...
	{
		simpleDB._recordTypes.forEach (function (recordType) {
			simpleDB.loadRecords (recordType);
		});
		return;
	}
...
SimpleDB.prototype.convertRecordTypeToSaveObject = function (recordType)
{
	...
	copyProperties (recordType,saveObject,['code','_index']);
...
SimpleDB.prototype.convertSaveObjectToRecordType = function (saveObject)
{
	...
	copyProperties (saveObject,recordType,['code','_index']);
	...
	for (var p in saveObject)
	{
		if (saveObject.hasOwnProperty (p))
		{
			...
			if (isArray (v))
			{
				...
				{
					var nop = findObjectInArray (recordType._nameObjectProperties,'propertyName',p,{UseTokenizedStrings: false});
					if (nop)
					{

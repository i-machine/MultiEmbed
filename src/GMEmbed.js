...
GMEmbedManager.prototype.initialiseEmbedItemTypes = function (rows)
{
	...
	this.addEmbedItemType ('youtube video',{
		...
		{
			if (item.player && item.player instanceof YT.Player)
				item.player.setVolume (Math.floor (volume))
		},
	})
	...
	this.addEmbedItemType ('twitch channel',{
		...
		getItemPlayerMute: function (item)
		{
			if (item.player && item.player instanceof Twitch.Player)
				return item.player.getMuted ()
			else
				return true
		},
		...
		{
			if (item.player && item.player instanceof Twitch.Player)
				item.player.setMuted (mute)
		},

		getItemPlayerVolume: function (item)
		{
			if (item.player && item.player instanceof Twitch.Player)
				return item.player.getVolume () * 100
			else
				return 0
		},
		...
		{
			if (item.player && item.player instanceof Twitch.Player)
				item.player.setVolume (volume / 100)
		},
...
GMEmbedManager.prototype.getEmbedItemTypeForUrl = function (url,parsedUrl)
{
	...
	for (var c = 0; c < this.itemTypes.length; c ++)
	{
		...
		if (playerModeAndId && gmValues.isDefinedAndNotNull (playerModeAndId.playerMode))
			return itemType
	}
...
GMEmbedManager.prototype.removePanel = function (panel,opRemoveItem)
{
	...
	if (panelItem)
		this.unbindItemFromPanel (panelItem)

	this.interfaceManager.removePanelDiv (panel)

	gmArrays.remove (this.panels,panel)

	if (panelItem && gmValues.getOptionalValue (opRemoveItem,true))
		this.removeItem (panelItem)
...
GMEmbedManager.prototype.normalisePanels = function ()
{
	...
	while (this.panels.length > requiredPanelsLength)
		this.removePanel (this.panels [this.panels.length - 1],false)
...
GMEmbedManager.prototype.normaliseItemPanels = function ()
{
	for (var c = 0; c < this.items.length; c ++)
	{
		...
		if (panel)
			this.bindItemToPanel (item,panel)
	}
}
...
GMEmbedManager.prototype.normaliseColumnsAndRows = function ()
{
	...
	if (!(this.autoColumns || this.autoRows))
		return
...
{
	return gmArrays.findElementUsingFilter (this.items,function (item) {return item.player === player})
}
...
GMEmbedManager.prototype.addItem = function (inputUrl)
{
	...
	if (!this.setItemUrl (item,inputUrl))
		return null

...
function initialiseEventHandlers ()
{
	...
	$(document).on ('mouseenter','.' + embedManager.interfaceManager.panelDivClass, function () {
		...
		if (panel)
		{
			...
			if (panel.item)
				$panelDiv.find ('.' + embedManager.interfaceManager.panelDivFloatingToolsDivClass).css ('display','block')
		}
	})
	...
	$(document).on ('mouseleave','.' + embedManager.interfaceManager.panelDivClass, function () {
		...
		{
			if (panel.item)
				$panelDiv.find ('.' + embedManager.interfaceManager.panelDivFloatingToolsDivClass).css ('display','none')
		}
	})
	...
	$(document).on ('keypress', function (event) {
		...
		var $focused = $(':focus')
		if (!((!$focused.length) || $focused.hasClass (embedManager.interfaceManager.panelDivClass)))
			return

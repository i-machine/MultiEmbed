...
GMEmbedInterfaceManager.prototype.addItem = function (inputUrl) {
    if (gmStrings.isEmpty(inputUrl)) {
        ...
        if (gmStrings.isEmpty(inputUrl)) {
            ...
            if (!inputUrl || gmStrings.isEmpty(inputUrl)) return null
        } else $('.urlInput').val('')
    }

    if (inputUrl.indexOf('http') !== 0) inputUrl = `https://www.twitch.tv/${inputUrl}`
...
GMEmbedInterfaceManager.prototype.addPanelDiv = function (panel) {
    ...
    html += sprintf('<span class="toggleItemVisible panelIcon clickable glyphicon glyphicon-facetime-video" title="toggle item visible"></span>')
...
GMEmbedInterfaceManager.prototype.createItemPlayer = function (item) {
    ...
    if (item.itemType) item.itemType.createItemPlayer(item, $playerDiv)
...
GMEmbedInterfaceManager.prototype.isItemMuted = function (item) {
    if (item.itemType && gmValues.isDefinedAndNotNull(item.itemType.getItemPlayerMute)) return item.itemType.getItemPlayerMute(item)
    else return true
}

GMEmbedInterfaceManager.prototype.setItemMute = function (item, mute) {
    if (item.player && item.itemType && gmValues.isDefinedAndNotNull(item.itemType.setItemPlayerMute)) item.itemType.setItemPlayerMute(item, mute)
}
...
GMEmbedInterfaceManager.prototype.isItemVisible = function (item) {
    if (item.itemType && gmValues.isDefinedAndNotNull(item.itemType.getItemPlayerVisible)) return item.itemType.getItemPlayerVisible(item)
    else return true
}

GMEmbedInterfaceManager.prototype.setItemVisible = function (item, visible) {
    if (item.player && item.itemType && gmValues.isDefinedAndNotNull(item.itemType.setItemPlayerVisible)) item.itemType.setItemPlayerVisible(item, visible)
}

GMEmbedInterfaceManager.prototype.toggleItemVisible = function (item) {
    this.setItemVisible(item, !this.isItemVisible(item))
}
...
GMEmbedInterfaceManager.prototype.setItemVolume = function (item, volume) {
    if (item.player && item.itemType && gmValues.isDefinedAndNotNull(item.itemType.setItemPlayerVolume)) item.itemType.setItemPlayerVolume(item, volume)
}

GMEmbedInterfaceManager.prototype.getItemVolume = function (item) {
    if (item.player && item.itemType && gmValues.isDefinedAndNotNull(item.itemType.getItemPlayerVolume)) return item.itemType.getItemPlayerVolume(item, volume)
    else return 0
}
...
GMEmbedInterfaceManager.prototype.normalisePanelDivs = function () {
    for (var c = 0; c < this.embedManager.panels.length; c++) {
        ...
        if (!panel.item || (this.maximizedPanel && this.maximizedPanel !== panel)) displayThisDiv = false
...
GMEmbedInterfaceManager.prototype.swapPanelItems = function (panel1, panel2) {
    ...
    if (panel1Item) this.embedManager.unbindItemFromPanel(panel1Item)
    if (panel2Item) this.embedManager.unbindItemFromPanel(panel2Item)

    if (panel1Item) this.embedManager.bindItemToPanel(panel1Item, panel2)
    if (panel2Item) this.embedManager.bindItemToPanel(panel2Item, panel1)
}

GMEmbedInterfaceManager.prototype.togglePanelMaximized = function (panel) {
    if (this.maximizedPanel === panel) this.maximizedPanel = null
    else this.maximizedPanel = panel
